...
=== Daemon build
In some cases you need to run the Maven build on the background. The first case is when the build itself starts some application (eg. `mvn spring-boot:run`). Second case is that you are building a project that is too big so can do some other operation in the meantime let it run on the background.
In these cases you can specify that the build should be ran as a daemon:
[source,java]
....
